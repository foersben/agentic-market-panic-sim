...
status: stable
version: 1.0
description: Official specification for the Open Knowledge Format v0.2.
...
| `customer_id` | STRING    | Foreign key into [customers](README.md). |
| `total_usd`   | NUMERIC   | Order total in US dollars.               |
| `placed_at`   | TIMESTAMP | When the customer submitted the order.   |

# Joins

Joined with [customers](README.md) on `customer_id`.
```
...
expected SLA. See the [orders table](README.md).
