...
\title{\textbf{AMPS: Agentic Market Panic Simulator}\\[0.2cm] \Large Generative Agent-Based Macroeconomic Engine for Financial Panic Dynamics}
...
\textbf{Base OS} & Ubuntu 26.04 LTS Bare-Metal & Linux 7.0 kernel, NVIDIA Driver 580+ Open, Docker CE \\ \hline
...
\begin{enumerate}
    \item \textbf{Local Self-Hosted GitHub Actions Runner:} Automate linting (\texttt{ruff}), secret scanning (\texttt{gitleaks}), Pydantic contract tests, and Numba LOB invariant verification (\texttt{Hypothesis}) directly on the local Ubuntu 26.04 server on every \texttt{git push}.
...
\begin{itemize}
    ...
    \item \textbf{Scope:} 
    \begin{itemize}
        \item \textit{DevOps \& Infrastructure:} Bare-metal provisioning of Ubuntu 26.04, securing network access via Tailscale, enabling ReBAR and disabling CSM in BIOS, and installing the open-source NVIDIA 580+ CUDA stack with \texttt{nvidia-container-toolkit} for Docker GPU passthrough.
