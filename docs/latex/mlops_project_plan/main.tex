...
% Nodes
\node[box] (agent) {\textbf{AGENT COGNITIVE ENGINE (SGLang)}\\ \scriptsize (Generative Policies / Reflexion)};
...
\textbf{Agent Cognitive Engine (SGLang):} At the top of the stack, the generative LLM agents reside. These agents receive the latest market state as a single, shared prompt prefix. Utilising the SGLang inference engine, the agents evaluate their individual personas and risk tolerances to generate discrete trading decisions. A built-in reflexion loop acts as an internal cognitive critique, forcing agents to evaluate their proposed actions against their historical behaviour before emitting the final structured JSON payload.
...
\filldraw[fill=green!10, draw=black, rounded corners] (0, -1.5) rectangle (4.5, -0.2) node[midway, align=center, font=\scriptsize] {Model Weights (1.58-Bit Ternary)\\Bonsai 27B ($\sim$7.5 GB)};
...
\begin{itemize}
    \item \textbf{Model Weights (7.5 GB):} Deploys a 1.58-bit ternary dense transformer (\texttt{Bonsai 27B} via BITCOS/PQ2\_0). Critically, this is not post-hoc compression of a standard model. Bonsai 27B is trained natively at 1.58-bit precision from initialisation --- the model's reasoning capacity and instruction-following are tuned at this precision level, preserving cognitive reliability that post-hoc quantisation of a standard model would destroy. Confining the entire parameter set to VRAM bypasses host-to-device PCIe serialisation bottlenecks entirely while unlocking massive 27B-parameter reasoning density.
...
Agent cognition is decoupled from standard request-reply pipelines, serving batch generations via the SGLang inference engine with RadixAttention and pre-compiled structural grammars (see Figure \ref{fig:cognitive_engine}).
...
To solve this, the pipeline utilises RadixAttention via the SGLang inference engine. Instead of treating each agent's request as an isolated generation, SGLang structures the prompts into a mathematical Radix Tree (a highly compressed prefix tree where nodes containing identical token sequences are merged). The shared 2,000-token macro environment acts as the root node of the tree and is computed and stored in the VRAM only \textit{once}. When the individual agents send their generation requests, SGLang traverses the Radix Tree, detects the identical prefix, and instantly routes all requests to read from that single cached memory block.

A deliberate architectural trade-off underlies this design: to preserve the KV-cache root hit across all 400 agents, every agent ingests the identical macro prompt at each tick. This symmetric broadcast maximises VRAM efficiency but eliminates asynchronous information diffusion across the agent population. Emergent herding ($H_t \to 1$) is therefore driven by persona-level heterogeneity in how each agent interprets the same shock, rather than by network topology or transmission delays. This is an explicit simplification of real-world rumour propagation dynamics. 

Consequently, only the highly specific, trailing tokens (the unique agent persona descriptors and their short JSON outputs) require active computation and isolated memory allocation per agent-branch. Personalized LanceDB RAG memories are appended strictly as per-agent suffix tokens after the shared root, not embedded within it --- preserving the Radix root node's validity for the full 400-agent cohort throughout the tick. The root node is invalidated and re-prefilled only when the God Agent generates a new macro shock, which occurs once per episode rather than once per tick. The computational savings are immense:
...
With the 2,000-token macro prompt evaluated once ($359\text{ MB}$ total), the 7.5 GB KV-cache budget can sustain approximately $42{,}740 / 100 \approx 400$ concurrent agent branches (averaging 100 unique generation tokens each) before triggering block eviction. Sustaining the full swarm of 1,000 agents would require a $\sim$18 GB KV-cache, exceeding the hardware ceiling; the simulation therefore operates with batches of up to $N_{\text{active}} = 400$ agents per tick.
    
\textbf{Zero-Overhead Constrained Decoding:} Agent actions must parse cleanly as structured order tickets. In a traditional pipeline, enforcing JSON output relies on post-generation validation (e.g., Pydantic throwing an error and triggering an expensive re-prompt). To solve this, the serving engine invokes XGrammar compiled directly into SGLang. By pre-compiling Pydantic action schemas into finite-state machine (FSM) token masks, invalid tokens are mathematically rejected at the logit level during CUDA generation. The model is physically forced to output valid JSON matching the schema, completely eliminating retry latency and token waste.
...
\begin{itemize}
    ...
    \item \textbf{Phase 2 (The Micro-Agent Swarm):} The 70B model is suspended, and the 1.58-bit ternary Bonsai 27B model (loaded entirely in the 16 GB GPU VRAM) processes up to 400 concurrent retail agent reactions per tick --- the hardware-bounded KV-cache ceiling.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Limit Order Book (LOB) Engine:} To prevent Python's Global Interpreter Lock (GIL) from bottlenecking high-throughput market clearing, the continuous crossover matching engine is implemented using compiled, memory-contiguous arrays (via Numba JIT with \texttt{@jit(nopython=True, cache=True)}). Enabling Numba's on-disk cache avoids the multi-second cold-start compilation delay on the first tick; a pre-warmed dummy dispatch is also executed before metric logging begins to prevent tick-0 telemetry outliers. It executes resting limit orders according to strict price-time priority:
    
    \[ V_{\text{executed}} = \min(Q_{\text{buy}}, Q_{\text{sell}}) \quad \text{where} \quad P_{\text{buy}} \ge P_{\text{sell}} \]
    
    With:
    \begin{description}
        \item[$V_{\text{executed}}$] The executed volume during a limit order cross.
        \item[$Q_{\text{buy}}$ / $Q_{\text{sell}}$] The quantity of the specific crossing limit orders.
        \item[$P_{\text{buy}}$ / $P_{\text{sell}}$] The limit prices. The new market price ($P_{\text{trade}}$) simply becomes the price of the resting limit order that was executed against.
    \end{description}
...
\textbf{Serving Node} & SGLang Docker Container & \texttt{--model-path <bonsai\_27b> --dtype int2 --mem-fraction-static 0.90} \\ \hline
\textbf{Pipeline Core} & Python 3.12 (pixi) & \texttt{uvloop}, \texttt{pydantic-core}, \texttt{lancedb}, \texttt{numba} \\ \hline
\textbf{Telemetry} & Prometheus \& Grafana & Scraping SGLang \texttt{/metrics} and custom LOB drift collectors \\ \hline
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{TTFT (Time-To-First-Token):} The latency between the event bus dispatching the prompt and SGLang yielding the first decision token.
...
To ensure the mathematical soundness of the simulation and the determinism of the LLM pipelines, a comprehensive test suite will be deployed and executed via GitHub Actions CI:

\begin{itemize}
    \item \textbf{Property-Based Testing (Hypothesis):} The Numba JIT Limit Order Book (LOB) matching engine handles extreme concurrency. Standard unit tests are insufficient to guarantee financial invariant preservation. The hypothesis library will generate millions of randomized, highly volatile order-flow scenarios to mathematically prove invariants (e.g., ensuring no liquidity is spontaneously created, and that the Best Bid strictly remains below the Best Ask).
    \item \textbf{Mutation Testing (mutmut):} To guarantee the resilience of the uvloop event bus and the meso-scale aggregation components, artificial bugs (mutations) will be systematically injected into the source code. The CI pipeline will measure whether the test suite accurately detects and fails on these hidden defects, ensuring high-fidelity test coverage.
    \item \textbf{Generative Parsing Tests:} The XGrammar JSON constraint masks will be stress-tested against adversarial LLM outputs. This ensures that even if an agent model enters an unexpected hallucination loop, the generation engine will mathematically reject the tokens rather than crashing the downstream Redis state.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textit{LLM Inference:} Deploying the SGLang container, downloading and serving the quantised models, tuning the Radix KV-Cache memory fraction to \texttt{--mem-fraction-static 0.90} (allocating $\sim$14.4 GB to SGLang and preserving $\sim$1.6 GB of CUDA driver headroom), and implementing XGrammar constrained JSON schemas.
...
\begin{itemize}
    \item \textbf{Phase 1 (Foundations):} Containerised SGLang environment via Docker; explicit system objectives and throughput metrics defined.
    \item \textbf{Phase 2 (Build \& Tracking):} End-to-end pipeline connecting the uvloop event bus to SGLang; MLflow tracking for generative model runs, Phased Inference parameter sweeps, and prompt registry versioning.
