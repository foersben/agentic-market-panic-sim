...
* **Target Function:** app/api/routers/config/trigger_rules.py and `config_trigger_rule_condition_node_update`
* **Selection Rationale:** The `config_trigger_rule_condition_node_update` function in `app/api/routers/config/trigger_rules.py` was selected because it had a high cognitive complexity score (32), but it essentially contained a long series of `if/elif` statements. Moving these out into a `_build_node_updates` private helper function drastically cut down the complexity (down to 11) without altering any engine logic.
...
* **Target Function:** `app/api/presenters/dashboard/payloads.py` - `build_live_dashboard_payload`
...
## 2025-02-28 - Complexity Refactoring Report
* **Target Function:** `app/api/presenters/dashboard/shared.py` `_describe_activation_condition`
...
* **Target Function:** `app/engine/core/flow_field.py` - `_compute_flow_field_impl`
...
* **Target Function:** `app/api/routers/telemetry.py` -> `export_telemetry_format`
...
* **Target Function:** `app/api/routers/telemetry.py` > `telemetry_chartjs_data`
